/* TPL: INICIO BLOQUE NUEVO [Lógica: preview + razas dinámicas + condicionales + guardado temporal] */
(function(){
  'use strict';

  // Catálogos compactos (si quieres listas completas, te preparo un JSON externo en el siguiente paso)
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const seguroRCData = byId('seguroRCData');
    const seguroRCComp = byId('seguroRCComp');
    const seguroRCNum = byId('seguroRCNum');
    ...
    // ==== Especie -> lista de razas/tipos (sin campo “tipo” aparte) ====
    ...
    // ==== Seguros: mostrar detalles solo si “Sí” ====
    function toggleSeguroDetails(sel, box, comp, num){
      const yes = sel.value === 'Sí';
      box.hidden = !yes;
      comp.required = num.required = yes;
      if (!yes){ comp.value = ''; num.value = ''; }
    }
    seguroVet.addEventListener('change', ()=>toggleSeguroDetails(seguroVet, seguroVetData, seguroVetComp, seguroVetNum));
    seguroRC.addEventListener('change', ()=>toggleSeguroDetails(seguroRC, seguroRCData, seguroRCComp, seguroRCNum));
    ...
    form.addEventListener('submit', (ev) => {
      ...
      const mascota = {
        ...
        microchip: microchipNo.checked ? 'No tiene' : (fd.get('microchip')||'').toString().trim(),
        ...
        seguroRCComp: (seguroRC.value==='Sí') ? (seguroRCComp.value||'') : '',
        seguroRCNum: (seguroRC.value==='Sí') ? (seguroRCNum.value||'') : '',

        // Foto (se setea abajo)
        foto: ''
      };

      // Validación extra: fechas coherentes
      if (mascota.desde && mascota.hasta && mascota.desde > mascota.hasta){
        alert('La fecha "desde" no puede ser posterior a "hasta".');
        return;
      }

      // Validaciones obligatorias estándar (HTML5 ya marca muchos con required)
