...
<head>
  ...
  <style>
    ...
    /* Robustez de ocultación por defecto */
    ...
    /* Acciones: submit centrado + ayuda derecha (sin mover ayuda) */
    .tpl-actions{
      display:grid;
...
<body>
  ...
  <div class="booking-wrapper">
    ...
    >
      ...
      <section class="tpl-section" aria-labelledby="sec-titular">
        ...
        <div class="booking-grid">
          ...
          <div class="booking-field">
            ...
            <input type="text" id="postalCode" name="CP" placeholder="Ej. 28001"
      ...
      <!-- Campos ocultos para email (resumen y mascotas) -->
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto-cachorro y contacto desde perfil (si existe)
    ...
    // Antes de enviar: preparar campo resumen (lo usa el correo)
    form.addEventListener('submit', async (e)=>{
      ...
      document.getElementById('summaryField').value = lines.join(' | ');
    });

    // Firebase: activar UI y auto-completar
    if (typeof firebase !== 'undefined') {
      const auth = firebase.auth();
      // TPL: persistencia local para que el login se comparta entre /perfil y /reserva
      try{ auth.setPersistence(firebase.auth.Auth.Persistence.LOCAL); }catch(_){}
      // iOS/Safari: recuperar redirect de Google si lo hubo
  ...
  <!-- TPL: INICIO BLOQUE NUEVO [Config EmailJS con tus IDs] -->
  <script>
    window.TPL_EMAILJS = {
  ...
  <!-- TPL: INICIO BLOQUE NUEVO [Submit robusto con EmailJS fijo + copia a cliente] -->
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      async function tryEmailJS(fd, extra){
        ...
        try{
          ...
          var template = cfg.templateId || cfg.template; // <-- usamos exactamente tu template_rao5n0c
          var pubKey   = cfg.publicKey || cfg.userId;
          var adminTo  = cfg.admin || 'gestion@thepetslovers.es';

          if(service && template){
            var payload = Object.fromEntries(fd.entries());
            Object.assign(payload, extra || {});
            if (!payload.to_email) payload.to_email = adminTo;

            // Enviar a gestión
            if (pubKey) { await emailjs.send(service, template, payload, pubKey); }
            else { await emailjs.send(service, template, payload); }

            // Copia al cliente si puso su email
            if ((payload.Email || payload.owner_email || '').includes('@')) {
              var clientTo = payload.Email || payload.owner_email;
              var clone = Object.assign({}, payload, { to_email: clientTo, reply_to: clientTo });
              if (pubKey) { await emailjs.send(service, template, clone, pubKey); }
              else { await emailjs.send(service, template, clone); }
            }
            return true;
          }
        }catch(err){
      ...
      form.addEventListener('submit', async function (e) {
        ...
        // Guardar en Firestore (si se puede) + mandar email siempre
        try {
          if (typeof firebase !== 'undefined' && firebase.firestore) {
            ...
            // Sin Firestore: al menos enviar el email
            var fdOnly = new FormData(form);
            await tryEmailJS(fdOnly, { _page: location.href, _tipo: 'reserva', _estado: 'enviada' });
          }
        } catch (err) {
          console.warn('TPL: no se pudo guardar la reserva en Firestore', err);
          // Aunque falle Firestore, intentamos el email igualmente
          try {
            var fd2 = new FormData(form);
            await tryEmailJS(fd2, { _page: location.href, _tipo: 'reserva', _estado: 'enviada' });
          } catch(_){}
        }

        // Éxito visual + redirección a perfil.html
        showSuccessOverlay();
      });
    });
  </script>
