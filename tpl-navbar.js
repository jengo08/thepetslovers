...
/* ===========================
   TPL: INICIO BLOQUE NUEVO [tpl-navbar — RESCATE SIMPLE y ESTABLE]
   =========================== */
(function(){
  if (window.__TPL_NAVBAR_BOOTED) return; window.__TPL_NAVBAR_BOOTED = true;

  // 🔧 CONFIG BÁSICA
  var PROFILE_URL = 'perfil.html'; // <- en minúsculas
  var PANEL_URL   = 'tpl-candidaturas-admin.html';
  var ADMIN_EMAILS = ['4b.jenny.gomez@gmail.com'];

  // Debug opcional: pon window.__TPL_DEBUG = true en consola si quieres logs.
  var DBG = !!window.__TPL_DEBUG;
  function log(){ if (DBG) try{ console.log.apply(console, ['[TPL]', ...arguments]); }catch(e){} }
  ...
  // Lee snapshot real de Auth (ignora anónimos)
  ...
  // Cache de email (para páginas sin Firebase activo)
  ...
  function mountNavbar(){
    try{
      ...
      markActiveLink();
      applySessionUI(true); // primer pintado
      firstBootRetry();     // ← REINTENTO inicial para Index
  ...
  // 🔎 Búsqueda robusta del “botón” de login/perfil/panel
  function queryLoginAnchor(){
    // 1) Preferimos .login-button
    var el = document.querySelector('.navbar .login-button');
    if (el) return el;

    // 2) Si no existe, buscamos por texto/href típico
  ...
  function setLoginButtonFromSnapshot(){
    try{
      var btn = queryLoginAnchor(); if(!btn){ log('No encuentro botón en .navbar'); return; }
  ...
  function applySessionUI(isFirst){
    ...
    try{
      if (window.firebase && firebase.auth){
        ...
        // si no hay Firebase, ya forzamos en firstBootRetry()
      }
    }catch(e){}
  }

  // ♻️ Reintento inicial (Index a veces llega antes del snapshot local)
  function firstBootRetry(){
    var tries = 0, maxTries = 10; // ~2–3s
  ...
  // 🔄 Actualizar al volver a la pestaña (por si se logueó en otra)
  document.addEventListener('visibilitychange', function(){
    if (!document.hidden) setLoginButtonFromSnapshot();
  });

  // 🔔 Cambios en localStorage (cierre/login en otra pestaña)
